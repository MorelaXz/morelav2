...
const SOAL_FILE    = path.join(process.cwd(), 'data/soal_susunkata.json')


let _soalCache: { index?: number; soal: string; tipe: string; jawaban: string }[] | null = null
...
function loadSoal() {
  ...
  try {
    ...
    console.error('[SUSUNKATA] Gagal load soal:', (e as Error).message)
  }
  return []
}
...
export function saveSessions(data: unknown) {
  try {
    ...
    console.error('[SUSUNKATA] Gagal simpan session:', (e as Error).message)
  }
}
...
const handler = async (m: any, { Morela, reply, fkontak }: any) => {
  ...
  try {
    ...
    if (!item) throw new Error('Bank soal kosong. Pastikan data/soal_susunkata.json ada.')

    const { soal, tipe, jawaban } = item
...
_© Morela Bot_`
    ...
    console.error('[SUSUNKATA ERROR]', (e as Error).message)
    await Morela.sendMessage(m.chat, { react: { text: '❌', key: m.key } })
    return reply(
`╭──「 ❌ *Gagal Ambil Soal* 」
│
